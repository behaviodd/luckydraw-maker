...
export default function VaultClient() {
  ...
  const duplicateDraw = async (id: string) => {
    const { data: original, error: fetchError } = await supabase
      .from('lucky_draws')
      .select('id, name, draw_button_label, probability_mode, is_active, draw_items(name, quantity, image_url, sort_order)')
      .eq('id', id)
      .single();
    ...
    const { data: newDraw, error: insertError } = await supabase
      .from('lucky_draws')
      .insert({
        user_id: user!.id,  // 원칙 2: 세션에서 추출, 파라미터 신뢰 금지
        ...
        {loading ? (
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="flex flex-col items-center py-20">
            <motion.div animate={{ y: [0, -8, 0], rotate: [0, 5, -5, 0] }} transition={{ duration: 3, repeat: Infinity }} className="mb-6">
              <div className="w-20 h-20 border-3 border-gum-black bg-gum-yellow/20 shadow-brutal flex items-center justify-center empty-icon">
                <Star className="w-10 h-10 text-gum-yellow" />
              </div>
            </motion.div>
            <p className="font-display text-xl text-gum-black mb-2">아직 럭키드로우가 없어요!</p>
